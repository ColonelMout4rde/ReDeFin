...
- ShellPage still has a **single page `Loader`**: every navigation, including Back, destroys and rebuilds the page and refetches. Keeping HomePage resident, a grid/detail cache and a `ListModel` for `moviepage` (`model: folderItems` is a JS array: each received page of 50 recreates every delegate) are identified but not done.
