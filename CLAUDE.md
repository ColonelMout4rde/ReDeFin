...
- The reveal decisions live in pure modules: `PageCurtainPolicy.js` (ShellPage: a page that declared `shellLoading` true then false is released at the first 60 ms tick; the 180 ms hold + 2 stable ticks only remain for a page that never declared anything), `GridRevealPolicy.js` (`moviepage` restore after a detail page: delegate placed + 150 ms of layout stability, was poster-ready + 1 100 ms), `DetailGatePolicy.js` (detail pages: `pageCanReveal`, and `nextUpGateReleased` which releases as soon as NextUpBlock is `ready` even when empty — a fully watched series used to wait for the 2.6 s timeout). Film, series and collection pages are divergent copies: fix all three in the same commit.
